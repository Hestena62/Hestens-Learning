...
<!-- Global Message Box -->
<div id="message-box" class="fixed inset-0 bg-black/75 backdrop-blur-sm hidden items-center justify-center z-[100]" role="alertdialog" aria-modal="true">
    <div class="bg-content-bg rounded-2xl shadow-2xl p-6 max-w-sm w-full text-center border animate-bounce-short">
        <div class="w-12 h-12 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-4 text-primary text-xl">
            <i class="fas fa-bell"></i>
        </div>
        <h4 id="message-title" class="text-lg font-bold mb-2 text-text-default">Notification</h4>
        <p id="message-text" class="mb-6 text-sm text-text-secondary leading-relaxed">Message content.</p>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const setup = (tid, pid, cid) => {
            ...
            const toggle = () => {
                ...
                else { p.classList.toggle('opacity-0'); p.classList.toggle('pointer-events-none'); p.classList.toggle('scale-90'); }
            };
            t.onclick = toggle;
